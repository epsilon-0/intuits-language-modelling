...
def IsotropyMetric1(
        percent, distMatrix, metric
):  # distMatrix is a 2d matrix that is n x n and tells you the distance between vector i and j
...
def getIsotropy1(M, metric):  # find the isotropy for an aribtrary metric
    return np.average(
        IsotropyMetric1(.1, distanceArbitraryMatrix(M, metric), metric))
...
def moveMatOnceNew(
        metric, M, i, j
):  # this is what we do if two words are said in the same context move them to increase their similarity
    global epsilon
    global alpha
    ...
    strengthI = 1. * cooccurrenceMat[i][j] / (
        1. * cooccurrenceMat[i][i])  # more common word, means it moves less
    ...
    if (norm(M[i]) > before and norm(M[j]) > before2):
        print(
            "the norm after for both, is larger than the one before \n \n \n ")
...
def moveMatOnce(
    ...
    if (metric(
            M[i], M[j]
    ) > prevMet):  # trying to get the vectors to move away from one another # if metric bigger, then more similar
        return
    else:
...
def solveforC(
        v1, v2, e1, e2
):  # v1 > v2 by construction , e1 and e2 are the perpindicular vectors constructed
    ...
    c = binarySearchSolution(
        lambda x: (norm(v1) - norm(v2) - norm(v2 - (2 * e2 * x))), 10**-14,
        -norm(v1), norm(v1))
    return c
...
def moveMatOnceCosineWay(
    ...
    vec2 = M[
        j]  # take which ever way vec 2 is, clockwise or counter clockwise to be positive theta
    a2 = norm(vec2)
    proj = np.inner(vec1, vec2) / (
        a1 * a1) * vec1  # projection of vector 2 onto vec1
...
def findOneClosestVectors(Mat, i, j, n, metric):  # look into pyfly
    ...
    dis = dis[dis[:, 0].argsort(
        kind='mergesort'
    )]  # sort by the 1st element (distance),  but we will return the actual index tho
    #print int(dis[np.random.randint(1,n)][1])
    return int(dis[np.random.randint(2, n + 1)][1])


def converseOnceAbout(
        M, N, i, j, metric
):  # converse about specific letters i and j (randomly choose a word that is similar to the previous words)
...
def converseAlotRandWalk(
        M, N, n, metric
):  # M is learner 1 # N learner 2; n is number of conversatoins; metric is the metrix used
    ...
    for a in range(0, n):
        ...
        if (count ==
            ...
            while (j == i):
                j = np.random.randint(0, M.shape[0] - 1)
        temp = i
        i = findOneClosestVectors(
            M, i, j, 20, metric
        )  # find a new word that is similar to word i, in the 1st speakers mind
        j = findOneClosestVectors(
            N, temp, i, 20, metric
        )  #find a new word that is similar to word i, in the 2nd speakers mind
        while (j == i):
...
def calcZ2(Mat):
    ...
    for i in range(0, Mat.shape[0]):
        if (cooccurrenceMat[i][i] == 0):
            continue
        logZ[i] = (norm(Mat[i])**2 /
                   (2 * Mat.shape[1])) - (np.log(cooccurrenceMat[i][i] /
                                                 (totalWordsSaid / 2)))
...
def entirePipelineRandWalk(
        metric,
        name):  # entire pipeline, but sentences are generally strucutured
    ...
    ZsEvery = 10
    Mat = generateNVectors(
        words, dimensions, -1. * rangeVecs,
        rangeVecs)  #words = 500,dimensions = 40, rangeVecs ={-10,10}
    Mat2 = generateNVectors(words, dimensions, -1. * rangeVecs, rangeVecs)
    if (wantPrint):
        printResults(Mat, "1" + metric.__name__ + "/1_" + name + "_RandWalk_" +
                     str(printEvery) + "_" + str(0) + ".txt", metric)
        printResults(Mat2, "1" + metric.__name__ + "/2_" + name + "_RandWalk_"
                     + str(printEvery) + "_" + str(0) + ".txt", metric)

    if (wantPrint):
        printIsotropy(
            printNorms(Mat),
            "1" + metric.__name__ + "/" + name + "averageNorms.txt")
        printIsotropy(
            "New Pipeline RANDOM",
            "1" + metric.__name__ + "/" + name + "RandWalk_kNN_isotropy.txt")
        printIsotropy(
            "New Pipeline RANDOM",
            "1" + metric.__name__ + "/" + name + "RandWalk_CLT_isotropy.txt")
    ...
    for i in range(1, NumConversations + 1):
        ...
        if (wantPrint):
            ...
            printIsotropy(a, "1" + metric.__name__ + "/" + name +
                          "RandWalk_kNN_isotropy.txt")
            printIsotropy(
                printNorms(Mat), "1" + metric.__name__ + "/" + name +
                "RandWalk_averageNorms.txt")
            printIsotropy(
                calcZ1(Mat) - calcZ2(Mat),
                "1" + metric.__name__ + "/" + name + "RandWalk_Zresults.txt")
            printResults(Mat,
                         "1" + metric.__name__ + "/1_" + name + "_RandWalk_" +
                         str(printEvery) + "_" + str(i) + ".txt", metric)
            printResults(Mat2,
                         "1" + metric.__name__ + "/2_" + name + "_RandWalk_" +
                         str(printEvery) + "_" + str(i) + ".txt", metric)
...
def entirePipeline(
    ...
    Mat2 = generateNVectors(words, dimensions, -1. * rangeVecs, rangeVecs)
    if (wantPrint):
        printResults(Mat, "1" + metric.__name__ + "/1_" + name + "_" +
                     str(printEvery) + "_" + str(0) + ".txt", metric)
        printResults(Mat2, "1" + metric.__name__ + "/2_" + name + "_" +
                     str(printEvery) + "_" + str(0) + ".txt", metric)
        ...
        printIsotropy(
            "New Pipeline RANDOM",
            "1" + metric.__name__ + "/" + name + "Random_kNN_isotropy.txt")
        printIsotropy(
            "New Pipeline RANDOM",
            "1" + metric.__name__ + "/" + name + "Random_CLT_isotropy.txt")
        printIsotropy(
            "Zresults Z value 1 way - Z value another (closer to 0 is best)",
    ...
    for i in range(1, NumConversations + 1):
        ...
        if (wantPrint):
            ...
            printIsotropy(
                printNorms(Mat),
                "1" + metric.__name__ + "/" + name + "Random_averageNorms.txt")
            printIsotropy(
                calcZ1(Mat) - calcZ2(Mat),
                "1" + metric.__name__ + "/" + name + "Random_Zresults.txt")
            printResults(Mat, "1" + metric.__name__ + "/1_" + name + "_" +
